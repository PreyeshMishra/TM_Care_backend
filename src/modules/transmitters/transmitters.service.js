...
async function get(
    ...
    const transmittersModels = await Transmitters.find(filter)
        .skip(queryParams.offset)
        .limit(queryParams.limit);

    const transmitters = transmittersModels.map((transmitter) =>
        transmitter.toJSON()
    );

    const count = await Transmitters.count(filter);
